...
class AudioVisualizer:
    def __init__(self, parent_frame, rate=22050, chunk=4096):
        ...
        # Configurar gráfico de tiempo (onda)
        ...
        # Configurar gráfico de frecuencias (espectro FFT)
    ...
    def update(self, audio_data, dominant_freq=None):
        ...
        # Convertir audio_data a numpy array si es necesario
        if isinstance(audio_data, bytes):
            audio_array = np.frombuffer(audio_data, dtype=np.int16)
        else:
            audio_array = np.array(audio_data, dtype=np.int16)

        # Actualizar buffer de audio
        ...
        # Normalizar magnitud para mejor visualización
        if len(magnitude) > 0:
            magnitude = magnitude / np.max(magnitude) if np.max(magnitude) > 0 else magnitude

        # Actualizar gráfico de tiempo (onda de audio)
